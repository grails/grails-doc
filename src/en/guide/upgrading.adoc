...
Domain class autowiring is disabled by default due to its impact on performance. You can re-enable autowiring for all domains using the `grails.gorm.autowire` setting in `application.yml` or just for specific domains like this:
