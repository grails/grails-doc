...
The new shell integrates closely with the concept of application profiles with each profile capable defining <<ref-comgrailsgrailsprofilerepositorytreemasterprofileswebcommands-profile specific commands,profile specific commands>>. As with previous versions of Grails, plugins can define new shell commands that can invoke Gradle or perform code generation and project automation tasks.

See the new guide on <<creatingCustomScripts,Creating Custom Scripts>> for more information.
