...
Services are typically involved with coordinating logic between <<GORM,domain classes>>, and hence often involved with persistence that spans large operations. Given the nature of services, they frequently require transactional behaviour. You can use programmatic transactions with the <<ref-domain-classes-withTransaction,withTransaction>> method, however this is repetitive and doesn't fully leverage the power of Spring's underlying transaction abstraction.
