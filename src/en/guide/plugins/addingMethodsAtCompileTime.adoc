...
`TraitInjector` can implement `SupportsClassNode` interface to apply traits to only those artefacts which satisfy some requirements.
For example, say you want to apply trait to an artefact only if artefact class has some annotation. it can be done as below
...
Above `TraitInjector` will add the `SomeTrait` to only those controllers which has SomeAnnotation declared.
