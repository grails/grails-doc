Grails controllers support the concept of command objects. A command object is a class that is used in conjunction with <<dataBinding,data binding>>, usually to allow validation of data that may not fit into an existing domain class.
