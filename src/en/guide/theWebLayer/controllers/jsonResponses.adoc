...
The `respond` method provides content negotiation strategies to intelligently produce an appropriate respond for the given client.
...
If you define a view (either a GSP or a link:webServices.html#jsonViews[JSON View]) then Grails render the view when using the `respond` method by calculating a model from the argument passed to `respond`.
...
If you wish to control the name of the model variable then you can pass the model directly using `respond` and a map argument:
...
You can also add additional values to the model by passing a `model` argument:
