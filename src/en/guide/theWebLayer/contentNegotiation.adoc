...
NOTE: When using <<ref-controllers-withFormat,withFormat>> make sure it is the last call in your controller action as the return value of the `withFormat` method is used by the action to dictate what happens next.
...
The <<ref-controllers-withFormat,withFormat>> available on controllers deals specifically with the response format. If you wish to add logic that deals with the request format then you can do so using a separate `withFormat` method available on the request:
...
To test content negotiation in a unit or integration test (see the section on <<testing,Testing>>) you can either manipulate the incoming request headers:
