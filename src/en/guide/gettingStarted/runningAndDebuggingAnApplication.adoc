Grails applications can be run with the built in Tomcat server using the <<ref-command-line-run-app,run-app>> command which will load a server on port 8080 by default:
