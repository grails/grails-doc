...
The above command will create a new Spock spec called `MyFunctionalSpec.groovy` in the `src/integration-test/groovy` directory. The test is annotated with the {apiDocs}grails/test/mixin/integration/Integration.html[Integration] annotation to indicate it is a integration test and extends the `GebSpec` super class:
