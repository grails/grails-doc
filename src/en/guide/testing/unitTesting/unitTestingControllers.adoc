...
The `xml` property is a parsed result from Groovy's http://groovy.codehaus.org/Reading+XML+using+Groovy's+XmlSlurper[XmlSlurper] class which is very convenient for parsing XML.
...
The unit testing environment respects the <<ref-controllers-allowedMethods,allowedMethods>> property in controllers.  If a controller action is limited to be accessed with certain request methods, the unit test must be constructed to deal with that.
