...
The most common place that you need messages is inside the view. Use the <<ref-tags-message,message>> tag for this:
...
The same technique can be used in <<taglibs,tag libraries>>, but if your tag library uses a custom <<namespaces,namespace>> then you must prefix the call with `g.`:
