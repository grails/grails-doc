...
The `Book` class has properties such as `title`, `releaseDate` and `author`. These can be used by the <<ref-domain-classes-findBy,findBy>> and <<ref-domain-classes-findAllBy,findAllBy>> methods in the form of "method expressions":
...
A method expression in GORM is made up of the prefix such as <<ref-domain-classes-findBy,findBy>> followed by an expression that combines one or more properties. The basic form is:
...
The same pagination and sorting parameters available on the <<ref-domain-classes-list,list>> method can also be used with dynamic finders by supplying a map as the final parameter:
