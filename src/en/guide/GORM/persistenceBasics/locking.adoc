...
In Grails pessimistic locking is performed on an existing instance with the <<ref-domain-classes-lock,lock>> method:
...
To get around this problem you can use the static <<ref-domain-classes-lock,lock>> method that takes an id just like <<ref-domain-classes-get,get>>:
...
As well as the <<ref-domain-classes-lock,lock>> method you can also obtain a pessimistic locking using queries. For example using a dynamic finder:
