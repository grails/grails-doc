...
The console is enabled by default in development mode and can be disabled or enabled in other environments by using the `grails.dbconsole.enabled` attribute in `application.yml` or `application.groovy`. For example, you could enable the console in production like this:
