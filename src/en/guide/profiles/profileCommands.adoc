...
If you need more flexiblity than what the declarative YAML approach provides you can create Groovy script commands. Each Command script is extends from the {apiDocs}org/grails/cli/profile/commands/script/GroovyScriptCommmand.html[GroovyScriptCommmand] class and hence has all of the methods of that class available to it.
