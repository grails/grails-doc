A profile can define new commands that apply only to that profile using YAML or Groovy scripts. Below is an example of the <<ref-command-line-create-controller,create-controller>> command defined in YAML:
