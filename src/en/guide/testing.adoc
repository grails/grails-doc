...
The first thing to be aware of is that all of the `create-\*` and `generate-\*` commands create `unit` or `integration` tests automatically. For example if you run the <<ref-command-line-create-controller,create-controller>> command as follows:
...
Tests are run with the <<ref-command-line-test-app,test-app>> command:
