...
A link:../../guide/theWebLayer.html#taglibs[tag library] is a "view helper" that in MVC terms helps the view with rendering by providing a set of reusable tags.
...
If you want the command to default to a different package for tag libraries, provide a value for `grails.project.groupId` in the link:../../guide/conf.html[runtime configuration].
