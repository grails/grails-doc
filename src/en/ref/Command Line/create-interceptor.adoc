...
A link:../../guide/theWebLayer.html#interceptors[interceptor] is responsible intercepting incoming web requests and performing actions such as authentication, logging and so on.
...
If you want the command to default to a different package for interceptors, provide a value for `grails.project.groupId` in the link:../../guide/conf.html[runtime configuration].
